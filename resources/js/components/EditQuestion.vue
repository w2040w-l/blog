<template>
  <div>
    <button class='btn btn-link float-right' v-on:click='popCreate'>{{ $root.tran('edit') }}</button>
    <popup ref='addQstn' iwidth='600' >
      <div method="post" v-on:click='closeToggle'>
        ...
        <button type="submit" class='btn btn-primary' v-on:click='edit'>{{ $root.tran('submit_question') }}</button>
      </div>
    </popup>
  </div>
</template>
